...
def get_status_view() -> Optional[Dict[str, Any]]:
    """Return the shared Fabric (azazel_fabric) StatusView if it is being emitted.

    Aligns the dashboard with Azazel-Fabric (formerly Azazel-Common) v0.3.0:
    when the shared view-model is present (azazel-fabric installed on the
    device, so the controller emits ui_status_view.json), the UI can render
    from the same StatusView shape Edge uses. Absent -> None, and callers
    fall back to the existing state fields, so this never changes behavior
...
def api_state():
    ...
    # Shared Fabric (azazel_fabric) StatusView (v0.3.0), None when not emitted.
