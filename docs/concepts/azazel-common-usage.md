# Using Azazel-Covenant in Azazel-Gadget

[`azazel-common`](https://github.com/01rabbit/Azazel-Covenant) — from
**Azazel-Covenant** (AZ-05, formerly Azazel-Common) — is the shared contract
...
> **Naming note:** the repository is now `01rabbit/Azazel-Covenant`. The pin
...
> ships. From Covenant `v0.3.0`, the dist becomes `azazel-covenant` and the
> import becomes `azazel_covenant`; `py/azazel_gadget/common_view.py` already
> tries `azazel_covenant` first and falls back to `azazel_common`, so Gadget
> is ready for that bump.

This page documents what Gadget uses from Covenant today and how it flows through
...
which currently pulls the tag from the renamed `01rabbit/Azazel-Covenant`
...
`azazel_covenant` first and falls back to `azazel_common` (see
...
- **Surface:** `/api/state` and the SSE stream include a `status_view` field
  (`null` when Covenant is not installed).
...
Gadget-specific rides through untouched, so aligning to Covenant never narrows
...
```bash
pip install -r requirements.txt   # includes azazel-common @ v0.2.0 (from 01rabbit/Azazel-Covenant)
```

On a developer machine, `bin/azazel-gadget-devstack up` runs the controller and
web with Covenant installed, so you can see `status_view` in `/api/state` and the
...
- **v0.3.0 namespace migration:** when Covenant ships `v0.3.0`, the dist
  renames to `azazel-covenant` and the import to `azazel_covenant`. Bump the
...
- **Future Covenant modules:** `azazel_common.paths` (Gadget's existing
  `path_schema` is the reference input), `azazel_common.audit`, and
  `azazel_common.notify` are later Covenant phases Gadget can adopt where they
