...
- **Azazel-CTI** (working name — its ADR-0001 defers the formal name) — an
  advisory-only, deterministic, on-prem tactical CTI knowledge-plane node. It never
  commands; it pairs with AZ-01 Azazel-Edge, which retains final decision authority
  and keeps working fully without it. AZ-02 Azazel-Gadget has no current or planned
  CTI integration. Reference: [01rabbit/Azazel-CTI](https://github.com/01rabbit/Azazel-CTI)
- **Azazel-Common** — the shared contracts library (distributed as `azazel-common`, installed via a pinned git tag) that
