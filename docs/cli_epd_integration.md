...
`py/azazel_gadget/cli_unified.py` は E-Paper ディスプレイと連動し、TUI の状態変化に応じて自動的にディスプレイを更新します。
...
```bash
sudo python3 py/azazel_gadget/cli_unified.py --enable-epd
```

### EPD 更新を無効化

```bash
python3 py/azazel_gadget/cli_unified.py --disable-epd
```
...
- `py/azazel_gadget/cli_unified.py` - TUI メインプログラム（EPD 連動機能含む）
- `py/azazel_epd.py` - EPD 制御スクリプト
- `icons/epd/` - EPD 用アイコンディレクトリ
- `fonts/` - EPD 用フォントディレクトリ
- `docs/azazel_epd_usage.md` - EPD 単体の使い方
